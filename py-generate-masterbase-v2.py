...
def gerar_uuid_master(componente: str, sistema: str, propriedade: str) -> str:
    """
    Gera um UUID determinístico (v5) baseado nas características do exame.
    Usa um namespace fixo para garantir consistência entre execuções.
    
    Args:
        componente: Nome do exame/procedimento
        sistema: Material de coleta (Sangue, Urina, etc)
        propriedade: Propriedade LOINC
    
    Returns:
        UUID string no formato: MK-{uuid}
    """
    # Namespace fixo para exames médicos (usando namespace DNS padrão)
    NAMESPACE_EXAMES = uuid.UUID('6ba7b810-9dad-11d1-80b4-00c04fd430c8')
    
    # String base normalizada (mesma lógica do hash para consistência)
    string_base = f"{normalizar_para_hash(componente)}|{normalizar_para_hash(sistema)}|{normalizar_para_hash(propriedade)}"
    
    # Gera UUID determinístico
    exam_uuid = uuid.uuid5(NAMESPACE_EXAMES, string_base)
    
    return f"MK-{exam_uuid}"
...
def criar_base_mestra_inicial(caminho_arquivo: str) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        ...
        # Agora usando UUID determinístico baseado nas características do exame
        # O código LOINC é mantido como atributo separado
        master_key = gerar_uuid_master(componente, sistema, propriedade)
